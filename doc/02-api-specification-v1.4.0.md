...
| 交通卡服务未注册 | `Transit card service is not registered` | `交通卡服务未注册` |
...
```json
{ "error": "NOT_FOUND", "message": "Key 'xxx' not found" }
```
...
| `recharge_execute` | 充值执行 | `sessionId` (string), `macSignature` (string) | `{ sw1, sw2, success }` |
...
| `print` | 打印文本 | `text` (string), `printerName` (string 可选) | `{ success: true }` |
| `print_raw` | 打印原始数据 | `data` (string hex), `printerName` (string 可选) | `{ success: true }` |
...
| printer | `job_completed` | `jobId`, `printerName` | 打印任务完成 |
| printer | `job_error` | `jobId`, `printerName`, `errorMessage` | 打印任务失败 |
| id-card | `card_inserted` | `readerName` | 身份证插入 |
| id-card | `card_removed` | `readerName` | 身份证拔出 |
| id-card | `reader_arrival` | `readerName` | 新读卡器接入 |
| id-card | `reader_removal` | `readerName` | 读卡器移除 |

插件驱动（如 `kacyber-go-card`）通过 `IHardwareWebSocketHandler` 注册后，其事件通过 `pcsc` target 推送给订阅了 `pcsc` 事件的 WS 客户端。
