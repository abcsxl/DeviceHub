...
通过 `Drivers:Pcsc:Mock` 配置项切换真实硬件与模拟实现。Mock 模式下 `MockPcscService` 提供完整的 `IPcscService` 接口模拟，无需物理读卡器即可开发和测试。
...
| Transmit | 根据 APDU 前缀返回固定响应（SELECT AID → `90 00` + 数据，GET BALANCE → `90 00` + 余额等） |
