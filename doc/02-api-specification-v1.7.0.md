...
| `READER_NOT_FOUND` | 读卡器未连接、已断开或被占用 |
| `PRINTER_NOT_FOUND` | 打印机不存在或未连接 |
| `IDCARD_NOT_FOUND` | 身份证阅读器不存在或未连接 |
| `TIMEOUT` | 硬件操作超时 |
| `HARDWARE_ERROR` | 未分类硬件异常 |

### PcscReader 专有错误码

PcscReader 插件产生的读卡器通信及卡片 APDU 执行错误。

| 错误码 | 说明 |
|--------|------|
| `CARD_NOT_PRESENT` | 读卡器中无卡片或卡片已移除 |
...
每 10 秒自动触发 `card_inserted` / `card_removed` 事件，`Mock Reader CL` 的卡片状态在 `card_present` 和 `empty` 之间切换。
...
#### GET `/api/hardware/pcsc/apdu-trace`
...
#### PUT `/api/hardware/pcsc/apdu-trace`
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| enabled | bool | 是 | true 启用 APDU 日志，false 关闭 |
...
APDU 跟踪日志写入 `data/logs/apdu.log`，仅记录 APDU 收发数据（不含其他调试信息），格式如下：
```
2026-07-24 10:30:00.123 [Reader Name] >> 00A4040008A00000063201010700
2026-07-24 10:30:00.234 [Reader Name] << 6F1C840E315041592E5359532E4444463031A50A5F4C0864617461536572769000
2026-07-24 10:30:00.345 [Reader Name] >> 805C000204
2026-07-24 10:30:00.456 [Reader Name] << 000000019000
```

`>>` 行为发送的 APDU 指令，`<<` 行为收到的响应数据 + SW 状态码。错误路径也记录：

```
2026-07-24 10:30:00.567 [Reader Name] << CONNECT ERROR READER_NOT_FOUND:Reader is not currently available
2026-07-24 10:30:00.678 [Reader Name] << TRANSMIT ERROR TIMEOUT:Operation timed out while communicating
```

默认关闭（`Drivers:Pcsc:ApduTraceEnabled: false`），可通过此端点运行时切换，无需重启服务。
...
日志管理提供运行时日志级别切换和文件日志配置。

命名规则：`/api/logs/...`。

#### GET `/api/logs`

通过 `level` 和 `tail` 参数过滤获取最近日志。

**请求参数**：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| level | string | 否 | 按日志级别过滤：DEBUG、INFORMATION、WARNING、ERROR。不传返回所有级别 |
| tail | int | 否 | 返回最近 N 条日志，范围 1-10000，默认 100 |
...
```json
{
  ...
  "data": {
    ...
    "entries": [
      {
        "timestamp": "2026-07-24T10:00:00.0000000Z",
        "level": "INFORMATION",
        "category": "DeviceHub.Service.Api.Program",
...
```json
{
  ...
  "data": {
    ...
    "DeviceHub.Cards.KaCyberGoCard": "Debug"
  }
}
```
...
运行时覆盖指定类别的日志级别。此修改立即生效，无需重启服务。

**请求体**：
```json
{
  "category": "DeviceHub.Cards.KaCyberGoCard",
  "level": "Debug"
}
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| category | string | 是 | 日志类别名称。设为 `Default` 影响所有未单独覆盖的类别 |
| level | string | 是 | 日志级别：Trace、Debug、Information、Warning、Error、Critical、None |
...
> 此覆盖仅在运行时生效，重启后丢失。如需持久化修改，请编辑 `appsettings.json` 的 `Logging:LogLevel` 节。

#### DELETE `/api/logs/levels`

移除指定类别的运行时日志级别覆盖，恢复为配置文件中的值。

**请求参数**：`?category=DeviceHub.Cards.KaCyberGoCard`
...
默认情况下日志仅保存在内存环形缓冲区（最大 1000 条）。如需持久化文件日志，在 `appsettings.json` 中启用：
...
启用后日志写入 `data/logs/devicehub-{yyyyMMdd}.log`，每日滚动。文件日志遵循 `Logging:LogLevel` 中的级别过滤配置。
...
|--------|-------|-----------|------|
| pcsc | `card_inserted` | `readerName`, `oldStatus`, `newStatus` | 卡片插入 |
| pcsc | `card_removed` | `readerName`, `oldStatus`, `newStatus` | 卡片拔出 |
...
| log | `log.debug` | `timestamp`, `level`, `category`, `message`, `exception` | 调试日志 |
| log | `log.info` | `timestamp`, `level`, `category`, `message`, `exception` | 信息日志 |
| log | `log.warning` | `timestamp`, `level`, `category`, `message`, `exception` | 警告日志 |
| log | `log.error` | `timestamp`, `level`, `category`, `message`, `exception` | 错误日志 |
...
| `POST /api/hardware/pcsc/apdu-trace` | REST | 配置查询/修改 |
| **交通卡 (TransitCard) 操作** | | |
...
|--------|-----------|---------|------|
| `DRIVER_NOT_FOUND` | 503 | 支持 | 驱动不存在或未注册 |
| `SERVICE_NOT_RUNNING` | 503 | 支持 | 服务未运行 |
| `INVALID_PARAMETERS` | 400 | 支持 | 请求参数错误或缺少必填字段 |
| `INVALID_ACTION` | 400 | 支持 | 操作的 action 不支持 |
| `READER_NOT_FOUND` | 404 | 支持 | 读卡器未连接、已断开或被占用 |
| `PRINTER_NOT_FOUND` | 404 | 支持 | 打印机不存在或未连接 |
| `IDCARD_NOT_FOUND` | 404 | 支持 | 身份证阅读器不存在或未连接 |
| `TIMEOUT` | 408 | 支持 | 硬件操作超时 |
| `HARDWARE_ERROR` | 500 | 支持 | 未分类硬件异常 |

### PcscReader 专有错误码

| 错误码 | HTTP 状态 | WS 场景 | 说明 |
|--------|-----------|---------|------|
| `CARD_NOT_PRESENT` | 404 | 支持 | 读卡器中无卡片或卡片已移除 |
| `FILE_NOT_FOUND` | 404 | 支持 | 文件未找到（SW=6A82/6A88） |
| `UNSUPPORTED_COMMAND` | 400 | 支持 | 不支持的命令（SW=6D00/6E00） |
| `SECURITY_ERROR` | 403 | 支持 | 安全状态不满足（SW=6982/6985/6988） |
| `INVALID_DATA` | 400 | 支持 | 数据无效（SW=6A80/6984） |
| `CARD_FULL` | 507 | 支持 | 卡片存储已满（SW=6A84） |

## 版本历史

| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.7.0 | 2026-07-24 | 新增 APDU 跟踪日志 API（GET/PUT /api/hardware/pcsc/apdu-trace），文件记录到 data/logs/apdu.log；新增运行时日志级别管理 API（GET/PUT/DELETE /api/logs/levels）；新增文件日志配置（Logging:File:Enabled 写入 data/logs/devicehub-{yyyyMMdd}.log）；新增 WS log 目标及 log.* 事件订阅 |
| v1.6.0 | 2026-07-24 | 移除多语言支持（Strings.resx、LocalizationExtensions、IStringLocalizer 全部剔除）；所有错误消息统一为英语硬编码；pcsc card_status_changed 拆分为 card_inserted/card_removed；实现全部 WS 事件；新增 PRINTER_NOT_FOUND/IDCARD_NOT_FOUND 错误码；TIMEOUT 归入通用错误码；重构错误码文档为通用+各插件专有分层结构；PrinterEndpoint PRINTER_NOT_FOUND→404，PRINT_FAILED 替换为 HARDWARE_ERROR；PcscEndpoint 新增 TIMEOUT 状态映射；新增 MapPcscStatus 统一映射 11 个 PCSC 错误码为对应错误码+英文描述；TransmitHex 异常消息改为 code:description 格式；修复多个 HandleError 解析 code:description |
