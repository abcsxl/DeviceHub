...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.6.0 | 2026-07-23 | pcsc card_status_changed 拆分为 card_inserted/card_removed；实现全部 WS 事件：pcsc（card_inserted/card_removed/reader_arrival/reader_removal），id-card（card_inserted/card_removed），printer（job_completed/job_error）；重构 MockIdCardService 支持事件轮询；新增 PrinterJobEventArgs 模型 |
